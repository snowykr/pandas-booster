...
use super::order::reorder_result_by_first_seen_u64;
use super::partition::{stable_scatter_by_partition, NUM_PARTITIONS};
use super::result::GroupByMultiResult;

type PartitionResults<K, V> = Vec<(Vec<(K, V)>, Vec<u64>)>;

pub(super) fn radix_groupby_engine_firstseen_u64<Ops, T, A, O>(
    ops: Ops,
    key_slices: &[&[i64]],
    values: &[T],
