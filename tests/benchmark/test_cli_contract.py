...
def test_readme_documents_explicit_smoke_reports_separately_from_default_full_generation():
    ...
    assert "# Run the lightweight checked-in smoke reports for all supported aggregations" in readme
    assert (
        "python benchmarks/generate_docs.py --samples 1 --cardinality standard --sort-mode sorted"
        in readme
    )
    assert "# Run publication-quality local reports for all supported aggregations" in readme
    assert "python benchmarks/generate_docs.py --samples 20" in readme
