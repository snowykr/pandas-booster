...
def render_standard_table(results: list[dict]) -> str:
    ...
    lines = []
    lines.append("| Workload | Groups | Sort | Type | Pandas | Polars | Booster |")
    lines.append("|----------|--------|------|------|--------|--------|---------|")
    ...
    for preset in preset_order:
        for sort_val in [True, False]:
            ...
            # Second row (Warm) - hide label, groups, sort.
            lines.append(
                f"|  |  |  | Warm | "
...
def validate_report_output_conflicts(output_dir: Path) -> None:
    ...
    conflicts = [
        ...
        if (output_dir / name).exists() and not is_generated_benchmark_report(output_dir / name)
    ]
    if conflicts:
...
def format_benchmark_document(
    ...
    correctness_section = format_correctness_section(results)
    stats_evidence_section = render_stats_evidence_section(stats_evidence)
    sections = [performance_section, correctness_section]
...
def save_results_md(
    ...
    """Save benchmark results as per-aggregation Markdown reports.

    Args:
        results: List of benchmark result dictionaries.
        output_path: Output directory path for generated Markdown reports.
    ...
    for agg in aggs:
        ...
        (output_dir / benchmark_report_filename(agg)).write_text(
            render_generated_markdown(report), encoding="utf-8"
        )

    (output_dir / BENCHMARK_INDEX_FILENAME).write_text(
        render_generated_markdown(format_benchmark_index(aggs)), encoding="utf-8"
    )
